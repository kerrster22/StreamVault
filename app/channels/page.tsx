...
import { Tv, Plus, Search, RefreshCw } from 'lucide-react'
...
export default function ChannelsPage() {
  ...
  const filterOptions = [
    ...
    { id: 'monitoring', label: 'Monitoring', count: channels.filter(c => c.monitoring).length },
    { id: 'not-monitoring', label: 'Not Monitoring', count: channels.filter(c => !c.monitoring).length },
  ]
  ...
  const monitoringCount = channels.filter(c => c.monitoring).length
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {channels.length} channels • {monitoringCount} monitoring
              </p>
            </div>
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center rounded-xl border border-border bg-card py-16">
      ...
      <p className="mt-1 text-sm text-muted-foreground">Save channels to track new uploads</p>
